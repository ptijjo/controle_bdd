...
const getSharedBrowser = jest.fn();

jest.mock('./puppeteer-browser', () => ({
  getSharedBrowser: (...args: unknown[]) => getSharedBrowser(...args),
...
describe('pdfCreator', () => {
  beforeEach(() => {
    ...
    getSharedBrowser.mockResolvedValue({
      newPage: browserNewPage,
    });
  });

  it('should reuse shared browser, render HTML and return a PDF buffer', async () => {
    const form = minimalForm({ client: 'casas', lieuControle: 'Gare Centrale' });

    const buffer = await generatePdf('martin', form);

    expect(getSharedBrowser).toHaveBeenCalled();
  ...
  it('should close the page when pdf generation fails', async () => {
